class ContactInfo::Admin::TerritorySupportedCountriesController < ApplicationController
        ...
        begin
          @territory_supported_countries.each do |territory_supported_country|
            begin
              ...
              # website.add_log(user: current_user, action: "Associated #{territory_supported_country.contact.name} with #{territory_supported_country.supported_country.name}")
              format.js
            rescue => e
  ...
  def update
    respond_to do |format|
      if @territory_supported_country.update_attributes(territory_supported_country_params)
  ...
  def destroy
    ...
    # website.add_log(user: current_user, action: "Removed a supported_country from #{@territory_supported_country.contact.name}")
  end  
