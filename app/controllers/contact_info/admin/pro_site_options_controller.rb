  ...
  def update
    respond_to do |format|
      if @pro_site_option.update(pro_site_option_params)
        ...
        format.xml  { head :ok }
        add_log(user: current_user, action: "Updated pro_site_options for #{@pro_site_option.contact.name}. showonweb:#{@pro_site_option.showonweb}, showforsolutions: #{@pro_site_option.showforsolutions}, showforchannels: #{@pro_site_option.showforchannels}")
      else
