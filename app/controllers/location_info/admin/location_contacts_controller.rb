class LocationInfo::Admin::LocationContactsController < LocationInfo::AdminController
        ...
        begin
          @location_contacts.each do |location_contact|
            begin
              ...
              add_log(user: current_user, action: "Associated #{location_contact.location.name} with #{location_contact.contact.name}")
              format.js
            rescue => e
  ...
  def update_order
    ...
    head :ok
    add_log(user: current_user, action: "Sorted location contacts")
  end   	
