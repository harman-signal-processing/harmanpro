namespace :attachments do
  ...
  task :move_paperclips => :environment do
    ...
    #old_path_interpolation = Paperclip::Attachment.default_options[:path]
    ...
    [Artist, Brand, CaseStudy, Event, Feature, LandingPage, NewsArticle, OnlineRetailer,
     Product, ReferenceSystem, Slide, TrainingContentPage, VerticalMarket].each do |klass|
      puts " #{ klass.to_s } ================= "
  ...
  desc "copy resource paperclip attachments to a new scheme"
  task :move_resource_paperclips => :environment do
    # Rackspace cloud files connection
    rackspace = Fog::Storage.new({
      provider:           'Rackspace',
      rackspace_username: ENV['RACKSPACE_USERNAME'],
