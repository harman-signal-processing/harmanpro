namespace :refresh do
  ...
  task :development_database do
    ...
    on roles(:db) do
      ...
      within folder do
        execute :mysqldump, "--add-drop-table --add-locks --create-options --disable-keys --lock-tables --quick --column-statistics=0 --set-charset --complete-insert -u #{@db['username']} --password=#{@env['harmanpro_database_password']} -h #{@db['host']} --port=#{@db['port']} #{@db['database']} > #{@filename}"
        curr = capture(:pwd)
